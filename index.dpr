...
var
  ...
  cokseviyorumulan : boolean;
  hayvangibihemde : boolean;
  indexChar : string;
...
var
  ...
  date,kler,ssg:TDateTime;
  T:TWord;
  n:integer;
  ok:boolean;
begin
  WordList := TList.Create;
  cokseviyorumulan := queryString='';
  buildindexstr;
  ...
  hayvangibihemde := s <> '';
  kler := 0;
  if hayvangibihemde then try
    kler := StrToDate(s);
  except
    hayvangibihemde := false;
  end;
    ...
    while not Eof(I) do begin
      if s <> '' then if s[1]='~' then begin
        ...
        if ok then begin
          ...
          if cokseviyorumulan then if date <> Trunc(Now) then continue else
            else if hayvangibihemde then if (date >= kler) and (date <= ssg) then continue;
